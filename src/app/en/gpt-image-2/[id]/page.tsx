...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildGptImage2CaseMetadata(item, "en");
}

export default async function EnGptImage2CaseDetailPage({ params }: Props) {
  const { id } = await params;
  const locale = "en";
  const cases = getAwesomePromptCases();
  const item = cases.find((c) => c.id === id);
          ...
          <div className="order-2 lg:order-1">
            ...
            <div className="flex items-center gap-4 text-sm text-text-muted mb-6">
              ...
              <span className="flex items-center gap-1.5">
                ...
                {item.prompt_language === "en" ? "English" : "Chinese"}
              </span>
            </div>
            ...
            <div className="border-t border-border-default pt-6">
              <h2 className="text-sm font-bold uppercase tracking-wider text-text-muted mb-3">Source Info</h2>
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-start gap-2">
                  <span className="text-text-muted shrink-0">Original Source:</span>
                  <a
