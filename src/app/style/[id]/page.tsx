...
export async function generateMetadata({
  ...
  const url = `https://promptstudio.art/style/${style.id}`;

  return {
    title: `${style.name_zh} — AI 图像风格模板`,
    description: `${style.description_zh} 包含 5 个示例提示词，复制模板即可使用。`,
    alternates: { canonical: url },
    openGraph: {
      title: `${style.name_zh} (${style.name_en})`,
      description: style.description_zh,
...
export default async function StyleDetailPage({
  ...
  const categoryLabel = CATEGORY_LABELS[style.category];

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: `${style.name_zh} — AI 图像风格模板`,
    description: style.description_zh,
    url: `https://promptstudio.art/style/${style.id}`,
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-6 sm:px-10 lg:px-16 pt-8 pb-20">
      ...
      <div className="mb-12 animate-fade-in-up stagger-2">
        <TemplateBlock template={style.template_prompt} />
      </div>
