...
export default function StyleCard({ style, index = 0, locale = "zh" }: { style: StyleTemplate; index?: number; locale?: Locale }) {
  const exampleCount = getExamplesByStyleId(style.id).length;
  const t = UI_TEXT[locale];
  const tags = locale === "zh" ? style.tags.slice(0, 3) : [style.category, "prompt", "template"];
